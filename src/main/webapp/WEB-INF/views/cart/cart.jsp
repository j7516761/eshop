	...
	<div class="container-fluid cart-container">
		...
		<c:if test="${not empty cartItems}">
			<c:forEach var="item" items="${cartItems}">
				<div class="cart-item">
					<img
						src="${pageContext.request.contextPath}/resources/images/products/${item.product.imageUrl}"
						class="cart-image" alt="${item.product.name}" />
					<!-- 正確的圖片路徑 -->
						...
						<div
							...
							<form action="updateQuantity.action" method="post"
								...
								<input type="number" name="quantity" value="${item.quantity}"
									...
									style="width: 100px; margin-right: 1rem;" /> <input
									type="hidden" name="productId" value="${item.product.id}" />
								<button type="submit" class="btn btn-success">Update</button>
							</form>
						</div>
						<a href="removeItem.action?productId=${item.product.id}"
